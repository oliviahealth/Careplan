...
type IMedicalServicesSubstanceUseInputs = z.infer<
  typeof MedicalServicesSubstanceUseInputsSchema
>;

const MedicalServicesSubstanceUseResponse =
  MedicalServicesSubstanceUseInputsSchema.extend({
    id: z.string(),
    user_id: z.string(),
  });
    ...
    const fetchUserData = async () => {
      if (submissionId) {
        try {
          ...
          MedicalServicesSubstanceUseResponse.parse(pastSubmissionData);
          ...
          Object.keys(pastSubmissionData).forEach((key) => {
            ...
            if (
              key === 'date_used_mat' ||
              key === 'date_used_medicine_service'
            ) {
          ...
          setShowMatDetails(pastSubmissionData.mat_engaged === 'Prior MAT use');
          setShowAddictionServiceDetails(
            pastSubmissionData.used_addiction_medicine_services === 'Prior Use'
          );
        } catch (error) {
  ...
  const { mutate } = useMutation(
    ...
    },
    {
      onSuccess: (data) => {
        ...
        console.log(
          `MedicalServicesForSubstanceUse data ${method} successfully.`,
          userData
        );
