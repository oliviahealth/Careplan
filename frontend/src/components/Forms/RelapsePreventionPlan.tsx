...
const RelapsePreventionPlanInputSchema = z.object({
  three_things_that_trigger_desire_to_use: z
    .string()
    .min(1, '3 things that trigger desire required'),
...
type IRelapsePreventionPlanResponse = z.infer<typeof RelapsePreventionPlanResponseSchema>;
...
export default function RelapsePreventionPlan() {
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<IRelapsePreventionPlanInput>({
  ...
  const addAnotherCaregiver = () =>
    append({ name: '', contact_number: '', relationship: '' });
  ...
  const { isFetching, refetch } = useQuery({
    ...
    onSuccess: (data: IRelapsePreventionPlanResponse) => {
      RelapsePreventionPlanResponseSchema.parse(data);

      Object.keys(data).forEach((key) => {
        if (key === 'id' || key === 'user_id') return;

        const formKey = key as keyof IRelapsePreventionPlanInput;
        setValue(formKey, data[key as keyof IRelapsePreventionPlanInput]);
      });
    },
  ...
  const { mutate: updateMutation, isLoading: isMutationLoading } = useMutation(
    async (data: IRelapsePreventionPlanInput) => {
      let responseData;
      let method;
      if (submissionId) {
    ...
    },
    {
      onSuccess: (data) => {
        ...
        setSuccessMessage(`Relapse Prevention Plan ${method} successfully!`);
        console.log(
          `RelapsePreventionPlan data ${method} successfully.`,
          userData
        );
    ...
    <div className="flex justify-center w-full p-2 mt-2 text-base font-OpenSans">
      ...
      >
        ...
        <p className="font-medium">
          List 3 things you know trigger your desire to use:
        </p>
        <textarea
          {...register('three_things_that_trigger_desire_to_use')}
          className="border border-gray-300 px-4 py-2 rounded-md w-full"
        />
        {errors.three_things_that_trigger_desire_to_use && (
          <span className="label-text-alt text-red-500">
            {errors.three_things_that_trigger_desire_to_use.message}
          </span>
        )}

        <p className="font-medium">
          List 3 skills or things you enjoy doing that can help get your mind
          off using:
        </p>
        <textarea
          {...register('three_skills_you_enjoy')}
          className="border border-gray-300 px-4 py-2 rounded-md w-full"
        />
        {errors.three_skills_you_enjoy && (
          <span className="label-text-alt text-red-500">
            {errors.three_skills_you_enjoy.message}
          </span>
        )}
        ...
        <textarea
          {...register('three_people_to_talk_to')}
          className="border border-gray-300 px-4 py-2 rounded-md w-full"
        />
        {errors.three_people_to_talk_to && (
          <span className="label-text-alt text-red-500">
            {errors.three_people_to_talk_to.message}
          </span>
        )}

        <p className="font-medium">
          In the case of a relapse, the patient's safe caregivers will be:
        </p>
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="space-y-4">
            ...
            <input
              {...register(`safe_caregivers.${index}.name`)}
              className="border border-gray-300 px-4 py-2 rounded-md w-full"
            />
            {errors.safe_caregivers && errors.safe_caregivers[index]?.name && (
              <span className="label-text-alt text-red-500">
                {errors.safe_caregivers[index]?.name?.message}
              </span>
            )}

            <p className="font-medium">Contact Number</p>
            <input
              {...register(`safe_caregivers.${index}.contact_number`)}
              className="border border-gray-300 px-4 py-2 rounded-md w-full"
            />
            {errors.safe_caregivers &&
              errors.safe_caregivers[index]?.contact_number && (
                <span className="label-text-alt text-red-500">
                  {errors.safe_caregivers[index]?.contact_number?.message}
                </span>
              )}

            <p className="font-medium">Relationship</p>
            <input
              {...register(`safe_caregivers.${index}.relationship`)}
              className="border border-gray-300 px-4 py-2 rounded-md w-full"
            />
            {errors.safe_caregivers &&
              errors.safe_caregivers[index]?.relationship && (
                <span className="label-text-alt text-red-500">
                  {errors.safe_caregivers[index]?.relationship?.message}
                </span>
              )}
          </div>
        ))}
        ...
        <p className="font-medium">
          Patient has Naloxone (opiod overdose reversal drug), and knows how to
          use it.
        </p>
        <div className="flex flex-col space-y-2">
          ...
          {errors.have_naloxone && (
            <span className="label-text-alt text-red-500">
              {errors.have_naloxone.message}
            </span>
          )}
        </div>
