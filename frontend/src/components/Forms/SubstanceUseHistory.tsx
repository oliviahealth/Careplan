...
export default function SubstanceUseHistory() {
  ...
  const handleDrugDate = (drug: keyof DrugVisibilityState, value: string) => {
    ...
    if (value === 'No') {
      setValue(
        `${drug}.date_last_used` as keyof ISubstanceUseHistoryInput,
        null
      );
    }
  };
          ...
          <div key={drug} className="pt-10">
            <div className="flex flex-nowrap space-x-4">
              ...
              <div className="flex flex-col space-y-2 flex-grow">
                ...
                {['Yes', 'No'].map((status) => (
                  <label key={status} className="inline-flex items-center">
                    <input
                      {...register(`${drug}.used_during_pregnancy`)}
                      className="mr-2"
                      type="radio"
                      value={status}
                    />
                ...
                {errors[drug]?.used_during_pregnancy && (
                  <span className="label-text-alt text-red-500">
                    {errors[drug]?.used_during_pregnancy?.message}
                  </span>
                )}
              </div>
            </div>
            {showDrugDate[drug] && (
              <>
                ...
                <input
                  {...register(`${drug}.date_last_used`)}
                  className="border border-gray-300 px-4 py-2 rounded-md w-full"
                  type="date"
                />
                {errors[drug]?.date_last_used && (
                  <span className="label-text-alt text-red-500">
                    {errors[drug]?.date_last_used?.message}
                  </span>
                )}
              </>
            )}
