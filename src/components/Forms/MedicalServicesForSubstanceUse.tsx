...
export default function MedicalServicesForSubstanceUse() {
    ...
    const { register, control, handleSubmit, formState: { errors }, setValue } = useForm<IMedicalServicesSubstanceUseInputs>({
        ...
        const fetchUserData = async () => {
            if (submissionId) {
                try {
                    ...
                    setShowAddictionServiceDetails(pastSubmissionData.used_addiction_medicine_services === 'Prior Use')
                } catch (error) {
    ...
    const { mutate } = useMutation(async (data: IMedicalServicesSubstanceUseInputs) => {
        let responseData;
        let method;
        if (submissionId) {
